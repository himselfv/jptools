...
interface
uses Classes, BalancedTree, UStrings;
...
type
 ...
 { Binary search node for phonetic syllables. In fact used only for Kana.
  ...
  public
    constructor Create(const AText: UnicodeString; ARule: PTranslationRule);
...

 {
  ...
  public
   ...
   { Supports inline markers:
    ...
    function RomajiToKana(const AString: string; AFlags: TResolveFlags): UnicodeString; override;
   { Someone might add an option to treat BIG LETTERS as katakana instead }
  end;

  TPinYinTranslator = class(TRomajiTranslator)
  protected
    function BopomofoBestMatch(const AText: UnicodeString): integer;
  public
    function KanaToRomaji(const AString: UnicodeString; AFlags: TResolveFlags): string; override;
    function RomajiToKana(const AString: string; AFlags: TResolveFlags): UnicodeString; override;
  end;
...
function ToHiragana(const ch: UnicodeChar): UnicodeChar; inline; overload;
function ToKatakana(const ch: UnicodeChar): UnicodeChar; inline; overload;
function ToHiragana(const s: UnicodeString): UnicodeString; overload;
function ToKatakana(const s: UnicodeString): UnicodeString; overload;
...
function fpytone(const i: byte): UnicodeChar; inline; //creates a character which encodes PinYin tone
...
function ConvertPinYin(const str: string): UnicodeString;
function DeconvertPinYin(romac: TPinYinTranslator; const str: UnicodeString):string;
...
function ToHiragana(const ch: UnicodeChar): UnicodeChar;
begin
...
function ToKatakana(const ch: UnicodeChar): UnicodeChar;
begin
...
function ToHiragana(const s: UnicodeString): UnicodeString;
var i: integer;
begin
  Result := '';
  for i := 1 to Length(s) do
    Result := Result + ToHiragana(s[i]);
end;

//Converts all hiragana in the string to katakana
function ToKatakana(const s: UnicodeString): UnicodeString;
var i: integer;
begin
  Result := '';
  for i := 1 to Length(s) do
    Result := Result + ToKatakana(s[i]);
end;
...
constructor TPhoneticRuleNode.Create(const AText: UnicodeString; ARule: PTranslationRule);
begin
  inherited Create;
  Self.FText := AText;
...
begin
  s_parts := SplitStr(s,',');
  Result.Phonetic := ToHiragana(s_parts[0]);
  base := 1;
  if ([EC_HIRAGANA,EC_KATAKANA] * EvalChars(s_parts[1]) <> []) then begin
    if ToHiragana(s_parts[1])<>Result.Phonetic then
...
function TKanaTranslator.SingleKanaToRomaji(var ps: PUnicodeChar; flags: TResolveFlags): string;
var bn: TBinTreeItem;
 {$IFDEF KATA_AT_SEARCH}
  chira: array[0..1] of UnicodeChar;
  pt: PUnicodeChar;
 {$ENDIF}
begin
 {$IFDEF KATA_AT_SEARCH}
...
function TKanaTranslator.KanaToRomaji(const AString: UnicodeString; AFlags: TResolveFlags): string;
var fn:string;
  s2:string;
  ps: PWideChar;
  i: integer;
...
function TPinYinTranslator.BopomofoBestMatch(const AText: UnicodeString): integer;
var i, cl: integer;
begin
  Result := -1;
  cl := 0;
  for i:=0 to FTrans.Count-1 do
    if pos(FTrans[i].Phonetic,AText)=1 then
      if Length(FTrans[i].Phonetic)>cl then
      begin
...
function TPinYinTranslator.KanaToRomaji(const AString: UnicodeString; AFlags: TResolveFlags): string;
var s2:string;
  cl:integer;
  i:integer;
  ch:WideChar;
  curstr:UnicodeString;
begin
  s2:='';
  curstr := AString;
  while curstr<>'' do
...
function fpytone(const i: byte): UnicodeChar;
begin
  Result := Chr(Ord(UH_PY_TONE)+i);
end;

//returns 0-5 if the character encodes tone, or 255
function fpydetone(const ch: UnicodeChar): byte;
begin
...
function fpygettone(const s: UnicodeString): byte;
begin
...
function fpyextrtone(var s: UnicodeString): byte;
begin
  Result := fpygettone(s);
...
function ConvertPinYin(const str:string): UnicodeString;
const UH_DUMMY_CHAR:UnicodeChar = #$F8F0;
  ...
  begin
   ...
   //relocate focus to new vowel in cases like "dnajianyang" (from dnA to jIan)
    ...
    begin
      ...
      ali:=copy(cnv2,length(cnv2)-(i-li-1)+1,i-li-1); //copy the rest of the output
      delete(cnv2,length(cnv2)-(i-li)+1,i-li); //delete char + rest from the output
